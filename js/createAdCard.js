...
const createAdCard  = ( advertisement ) => {
  ...
  advertisement.offer.address.randomLat && advertisement.offer.address.randomLng ? ad.querySelector('.popup__text--address').textContent = `${advertisement.offer.address.lat}' Tōkyō-to, Chiyoda-ku, Ichibanchō, '${advertisement.offer.address.lng}` : ad.querySelector('.popup__text--address').remove();
  ...
  if( advertisement.offer.features ){
    advertisement.offer.features.forEach((featureTitle) => {
      ...
      li.classList.add('popup__feature', `popup__feature--${featureTitle}`);
